...
export async function updateProduct(id: string, productData: Partial<Product>): Promise<Product | null> {
    ...
    if (fields.length === 0) {
        ...
        const [rows] = await db.execute<RowDataPacket[]>('SELECT product_uuid as id, name, category, price, cost_price as costPrice, stock, unit_of_measure as unitOfMeasure, image_url as imageUrl, description FROM products WHERE product_uuid = ?', [id]);
    ...
    const [updatedRows] = await db.execute<RowDataPacket[]>('SELECT product_uuid as id, name, category, price, cost_price as costPrice, stock, unit_of_measure as unitOfMeasure, image_url as imageUrl, description FROM products WHERE product_uuid = ?', [id]);
