...
export async function GET(req: NextRequest, { params }: RouteParams) {
    try {
        ...
        console.error(`--- DATABASE ERROR: GET /api/products/${params.id} ---`);
        console.error(error);
...
export async function PUT(req: NextRequest, { params }: RouteParams) {
  try {
    ...
    console.error(`--- DATABASE ERROR: PUT /api/products/${params.id} ---`);
    console.error(error);
    console.error('--- END DATABASE ERROR ---');
...
export async function DELETE(req: NextRequest, { params }: RouteParams) {
  try {
    ...
    console.error(`--- DATABASE ERROR: DELETE /api/products/${params.id} ---`);
    console.error(error);
    console.error('--- END DATABASE ERROR ---');
    ...
    try {
        ...
        if (stockChange < 0) {
            ...
            if (currentStockRows[0].stock < Math.abs(stockChange)) {
                return NextResponse.json({ message: `Not enough stock for product ${id}. Available: ${currentStockRows[0].stock}, Requested: ${Math.abs(stockChange)}` }, { status: 409 });
            }
        }
        ...
        console.error(`--- DATABASE ERROR: PATCH /api/products/${params.id} ---`);
        console.error(error);
        console.error('--- END DATABASE ERROR ---');
