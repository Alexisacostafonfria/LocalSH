...
// src/components/orders/OrderTicket.tsx
"use client";

import React from 'react';
import Image from 'next/image';
import { Order, AppSettings, BusinessSettings, SaleItem, ORDER_STATUS_MAP } from '@/types';
...
const OrderTicket: React.FC<OrderTicketProps> = ({ order, appSettings, businessSettings }) => {
  ...
  return (
    <div className="p-1 bg-white text-black text-xs w-full font-mono">
      ...
      <div className="mb-2">
        ...
        <p>Estado: {ORDER_STATUS_MAP[order.status]}</p>
      </div>
      ...
      <div className="grid grid-cols-12 gap-1 font-semibold mb-1">
        ...
        <div className="col-span-8 text-left">Producto</div>
      ...
      {order.items.map((item: SaleItem) => (
        <div key={item.productId} className="grid grid-cols-12 gap-1 py-0.5">
          ...
          <div className="col-span-8 text-left truncate" title={item.productName}>{item.productName}</div>
      ...
      <div className="text-center mt-3">
        <p>Presente este ticket para retirar su pedido.</p>
